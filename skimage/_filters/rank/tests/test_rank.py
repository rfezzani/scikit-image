...
all_rank_filters = [autolevel, equalize, gradient, maximum, mean,
                    geometric_mean, subtract_mean, median, minimum,
                    modal, enhance_contrast, pop, threshold,
                    noise_filter, entropy, otsu, sum,
                    windowed_histogram, majority,
...
class TestRank:
    ...
    @parametrize('func', all_rank_filters)
    def test_rank_filter(self, func):
        @test_parallel(warnings_matching=['Possible precision loss'])
        def check():
            expected = self.refs[func.__name__]
            result = func(self.image, self.selem)
            if func.__name__ == "entropy":
    ...
    def test_random_sizes(self):
        ...
        for m, n in np.random.randint(1, 101, size=(10, 2)):
            ...
            mean(image=image8, selem=elem, mask=mask, out=out8,
                 shift_x=0, shift_y=0)
            ...
            geometric_mean(image=image8, selem=elem, mask=mask, out=out8,
                           shift_x=0, shift_y=0)
            assert_equal(image8.shape, out8.shape)
            geometric_mean(image=image8, selem=elem, mask=mask, out=out8,
                           shift_x=+1, shift_y=+1)
            ...
            mean(image=image16, selem=elem, mask=mask, out=out16,
                 shift_x=0, shift_y=0)
            ...
            geometric_mean(image=image16, selem=elem, mask=mask, out=out16,
                           shift_x=0, shift_y=0)
            assert_equal(image16.shape, out16.shape)
            geometric_mean(image=image16, selem=elem, mask=mask, out=out16,
                           shift_x=+1, shift_y=+1)
            ...
            mean_percentile(image=image16, mask=mask, out=out16,
                            selem=elem, shift_x=0, shift_y=0, p0=.1, p1=.9)
            assert_equal(image16.shape, out16.shape)
            mean_percentile(image=image16, mask=mask, out=out16,
                            selem=elem, shift_x=+1, shift_y=+1,
                            p0=.1, p1=.9)
            assert_equal(image16.shape, out16.shape)
    ...
    def test_compare_with_grey_dilation(self):
        ...
        for r in range(3, 20, 2):
            ...
            maximum(image=image, selem=elem, out=out, mask=mask)
    ...
    def test_compare_with_grey_erosion(self):
        ...
        for r in range(3, 20, 2):
            elem = np.ones((r, r), dtype=np.uint8)
            minimum(image=image, selem=elem, out=out, mask=mask)
    ...
    def test_bitdepth(self):
        ...
        for i in range(8, 13):
            ...
            with expected_warnings(expected):
                mean_percentile(image=image, selem=elem, mask=mask,
                                out=out, shift_x=0, shift_y=0,
                                p0=.1, p1=.9)
    ...
    def test_population(self):
        ...
        pop(image=image, selem=elem, out=out, mask=mask)
    ...
    def test_compare_autolevels(self):
        ...
        loc_perc_autolevel = autolevel_percentile(image, selem=selem,
                                                  p0=.0, p1=1.)
    ...
    def test_compare_8bit_vs_16bit(self, func):
        # filters applied on 8-bit image ore 16-bit image (having only real 8-bit
        # of dynamic) should be identical
    ...
    def test_trivial_selem8(self):
        ...
        mean(image=image, selem=elem, out=out, mask=mask,
             shift_x=0, shift_y=0)
        assert_equal(image, out)
        geometric_mean(image=image, selem=elem, out=out, mask=mask,
                       shift_x=0, shift_y=0)
        assert_equal(image, out)
        minimum(image=image, selem=elem, out=out, mask=mask,
                shift_x=0, shift_y=0)
    ...
    def test_entropy(self):
        ...
        with expected_warnings(['Bad rank filter performance']):
            assert(np.max(entropy(data, selem)) == 12)

        # make sure output is of dtype double
        with expected_warnings(['Bad rank filter performance']):
            out = entropy(data, np.ones((16, 16), dtype=np.uint8))
    ...
    def test_selem_dtypes(self):
        ...
        for dtype in (np.bool_, np.uint8, np.uint16, np.int32, np.int64,
            ...
            geometric_mean(image=image, selem=elem, out=out, mask=mask,
                           shift_x=0, shift_y=0)
            assert_equal(image, out)
            mean_percentile(image=image, selem=elem, out=out, mask=mask,
                            shift_x=0, shift_y=0)
            assert_equal(image, out)
    ...
    def test_16bit(self):
        ...
        for bitdepth in range(17):
            ...
            if bitdepth >= 11:
                ...
                expected = []
            with expected_warnings(expected):
                assert minimum(image, selem)[10, 10] == 0
                assert maximum(image, selem)[10, 10] == value
                assert mean(image, selem)[10, 10] == int(value / selem.size)
    ...
    def test_bilateral(self):
        ...
        assert mean_bilateral(image, selem, s0=1, s1=1)[10, 10] == 1000
        assert pop_bilateral(image, selem, s0=1, s1=1)[10, 10] == 1
        assert mean_bilateral(image, selem, s0=11, s1=11)[10, 10] == 1005
        assert pop_bilateral(image, selem, s0=11, s1=11)[10, 10] == 2
    ...
    def test_percentile_min(self):
        ...
        selem = disk(15)
        # check for 8bit
        img_p0 = percentile(img, selem=selem, p0=0)
        img_min = minimum(img, selem=selem)
        assert_equal(img_p0, img_min)
        # check for 16bit
        img_p0 = percentile(img16, selem=selem, p0=0)
        img_min = minimum(img16, selem=selem)
        assert_equal(img_p0, img_min)
    ...
    def test_percentile_max(self):
        ...
        selem = disk(15)
        # check for 8bit
        img_p0 = percentile(img, selem=selem, p0=1.)
        img_max = maximum(img, selem=selem)
        assert_equal(img_p0, img_max)
        # check for 16bit
        img_p0 = percentile(img16, selem=selem, p0=1.)
        img_max = maximum(img16, selem=selem)
        assert_equal(img_p0, img_max)
    ...
    def test_percentile_median(self):
        ...
        selem = disk(15)
        # check for 8bit
        img_p0 = percentile(img, selem=selem, p0=.5)
        img_max = median(img, selem=selem)
        assert_equal(img_p0, img_max)
        # check for 16bit
        img_p0 = percentile(img16, selem=selem, p0=.5)
        img_max = median(img16, selem=selem)
        assert_equal(img_p0, img_max)
    ...
    def test_sum(self):
        ...
        sum(image=image8, selem=elem, out=out8, mask=mask)
        assert_equal(r, out8)
        sum_percentile(
        ...
        sum(image=image16, selem=elem, out=out16, mask=mask)
    ...
    def test_windowed_histogram(self):
        ...
        windowed_histogram(image=image8, selem=elem, out=outf, mask=mask)
        assert_equal(r0, outf[:, :, 0])
        assert_equal(r1, outf[:, :, 1])

        # Test n_bins parameter
        larger_output = windowed_histogram(image=image8, selem=elem,
                                                mask=mask, n_bins=5)
    ...
    def test_median_default_value(self):
        ...
        assert_equal(median(a), median(a, full_selem))
        assert median(a)[1, 1] == 0
        assert median(a, disk(1))[1, 1] == 1

    def test_majority(self):
        img = camera()
        elem = np.ones((3, 3), dtype=np.uint8)
        expected = windowed_histogram(
            img, elem).argmax(-1).astype(np.uint8)
        assert_equal(expected, majority(img, elem))
    ...
    def test_input_boolean_dtype(self):
        ...
        with raises(ValueError):
            maximum(image=image, selem=elem)

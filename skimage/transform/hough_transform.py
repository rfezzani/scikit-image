...
from ..measure import label, regionprops
...
def _prominent_peaks(image, min_xdistance=1, min_ydistance=1,
    ...
    ndi.maximum_filter1d(img_max, size=xcoords_size, axis=1,
                         mode='constant', cval=0, output=img_max)
    img *= (img == img_max)
    ...
    coords = np.round([p.centroid for p in props]).astype(int)
